...
\begin{abstract}
We present the $\theta$-calculus, an extension of the untyped lambda calculus
...
polymorphism, substructural types, and dependent types all arise as
...
substructural types, and dependent types all emerge from the same computational
...
\item \textbf{Unified type system hierarchy.} Simply Typed Lambda Calculus,
  System~F, substructural types, and dependent types are all encoded using the
...
operation that inspects and decomposes the layered structure of a term.
...
Section~\ref{sec:encodings} demonstrates that STLC, System~F, substructural
types, and dependent types are all encodable in the calculus.
...
\textsc{E-Ground}, producing a terminal selector. For multi-layer sub-terms,
...
each correspondence. Since lambda calculus is turing complete, it can simulate memory operations, 
conditional expressions, and error-signalling keywords. 
For the sake of simplicity, we introduce this definitions to show how type system encodings are constructed.

Predefined functions:
we use camel case for predefined functions.
for completeness, predifined functions are introduces as followings:
$(\lambda Add. \lambda Sub. \text{"pseudo code uses Add or Sub"}) AddImpl SubImpl$
...
G &\coloneqq& \lambda a.\,\lambda b.\
  \text{if } b <: a \text{ then } a \text{ else } \mathit{TypeError}
\end{array}
$$

$I$ is the identity combinator. $B$ is the composition combinator:
$B\ f\ g\ x = f\ (g\ x)$. $G$ is a type guard that takes two arguments:
$a$, the formal parameter type declared in the function signature, and $b$,
the type of the argument actually passed. $G$ returns $a$---the declared
type---making the type check independent of the argument's type.

For simplicity, the encodings in this section use pseudocode conventions
that are not part of the formal $\theta$-calculus rules defined in
Sections~\ref{sec:evaluation} and~\ref{sec:separation}. In particular, we
freely use conditional expressions ($\text{if}/\text{then}/\text{else}$),
error-signalling keywords ($\mathit{TypeError}$), a subtyping relation
($<:$), predefined base types ($\mathit{Int}$, $\mathit{Str}$), literal
values ($1$, $2$, \ldots), and native arithmetic operators ($+$). These are
meta-level devices used to state the \emph{intent} of each guard combinator
concisely; a full formalization would encode them as $\theta$-calculus terms
or treat them as an ambient theory.
...
at the type-checking layer (check that the argument is a subtype of $T$ and
return $T$). The composition combinator $B$ threads the argument through both
layers.

\subsubsection*{Worked example}

Consider the STLC term $(\lambda x{:}\mathit{Int}.\, x + 1)\ 2$.
...
When the result of type checking must depend on the argument's type---for
example, to track whether a file handle is open or closed---we use a different
guard combinator:

$$
G_b \coloneqq \lambda a.\,\lambda b.\
  \text{if } b <: a \text{ then } b \text{ else } \mathit{TypeError}
$$

$G_b$ returns the \emph{input} type $b$ rather than the annotation type $a$,
preserving precise type information through the computation. The encoding is
otherwise identical:
...
The only difference between STLC and substructural typing is the guard
combinator. $G$ discards the precise input type (returning the annotation),
while $G_b$ preserves it. This distinction controls how type information flows
through the system, which is the essence of substructural typing.
...
\noindent where $\mathcal{G} \in \{G,\, G_b,\, G_D\}$. The layering
...
Substructural & $G_b$ & input type $b$ \\
...
\textsc{E-Ground} produces a terminal selector. For separable bodies,
...
determines whether the system behaves as STLC ($G$), supports substructural
tracking ($G_b$), or handles dependent types ($G_D$). This suggests that the
...
type system features. Simply Typed Lambda Calculus, System~F, substructural
types, and dependent types are not distinct formalisms; they are
configurations of the same system, differing only in the choice of guard
combinator ($G$, $G_b$, or $G_D$).
...
In \textit{Advanced Topics in Types and Programming Languages},
chapter~1, pages 3--43. MIT Press, 2005.
