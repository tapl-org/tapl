...
\title{The $\theta$-Calculus:\\Unifying Type Systems through Layered Computation}
...
defines the combinators used in the encodings and demonstrates each
correspondence.
...
For simplicity, $G$ uses $\text{if}$ clauses and a $\mathit{TypeError}$
keyword. We also use pseudocode for predefined types such as $\mathit{Int}$
and $\mathit{Str}$, and the addition operator $+$.
...
\section{Formal Rules}\label{sec:appendix}
