...
\emph{unlayering} ($\theta.\,t$)---and shows that simply type checking, polymorphism, type constructors,
...
\item \textbf{Unified type system hierarchy.} Simply Typed Lambda Calculus,
  System~F, Type Constructors, and dependent types are all encoded using the
...
If the body is single-layer, wrap it in a selector:

$$
\dfrac{\epsilon[\theta.\,g]}{\lambda x.\, g}
\quad (\textsc{E-Ground})
$$

A single-layer term has no layers to decompose, so unlayering produces a
constant selector that ignores its argument and returns $g$ directly.
...
Multi-layer terms are stuck under evaluation---they do not reduce and must be
explicitly unlayered to make progress:
