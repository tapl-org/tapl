...
class Dog_:
    ...
    def bark(self):
        ...
        with s1.tapl_typing.scope_forker(s1) as f1:
            s2 = s1.tapl_typing.fork_scope(f1)
            s2.self
            s2.tapl_typing.add_return_type(s2, s2.self.name + s2.Str)
            s2 = s1.tapl_typing.fork_scope(f1)
...
def make_dog(factory, name):
    ...
    s1.make_dog = s1.tapl_typing.create_function([s1.Dog_, s1.Str], s1.Dog)
    s1.tapl_typing.set_return_type(s1, s1.Dog)
    s1.tapl_typing.add_return_type(s1, s1.factory(s1.name))
    return s1.tapl_typing.get_return_type(s1)
s0.make_dog = s0.tapl_typing.create_function([s0.Dog_, s0.Str], make_dog(s0.Dog_, s0.Str))
...
def main():
    ...
    s1.new_dog = s1.make_dog(s1.Dog_, s1.Str)
