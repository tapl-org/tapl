...
def test_assign_empty_list():
    ...
    assert ast.unparse(stmt2) == 's0.a = s0.tapl.typing.create_typed_list()'
    assert run_stmt([stmt2]) is None
    assert run_stmt([stmt1]) is None
...
def test_return1():
    ...
    assert ast.unparse(stmt2) == 's0.tapl.typing.add_return_type(s0, s0.NoneType)'


def test_return2():
    [stmt1, stmt2] = parse_stmt('return True')
    assert ast.unparse(stmt1) == 'return True'
    assert ast.unparse(stmt2) == 's0.tapl.typing.add_return_type(s0, s0.Bool)'
...
def hello():
    ...
    s1.tapl.typing.add_return_type(s1, s1.Int)
    return s1.tapl.typing.get_return_type(s1)
s0.hello = s0.tapl.typing.create_function([], hello())
""".strip()
    )
...
def hello():
    s1 = s0.tapl.typing.create_scope(parent__sa=s0)
    s1.hello = s1.tapl.typing.create_function([], s1.Int)
...
""".strip()
    ...
    assert (
        ...
        == """
def area(radius):
    s1 = s0.tapl.typing.create_scope(parent__sa=s0, radius=radius)
    s1.tapl.typing.add_return_type(s1, s1.Float * s1.radius * s1.radius)
    return s1.tapl.typing.get_return_type(s1)
s0.area = s0.tapl.typing.create_function([s0.Int], area(s0.Int))
""".strip()
    )
...
""".strip()
    ...
    assert (
        ...
        == """
def greet(name):
    s1 = s0.tapl.typing.create_scope(parent__sa=s0, name=name)
...
class Circle:
    ...
    def __init__(self, radius):
        ...
        return s1.tapl.typing.get_return_type(s1)
s0.Circle = s0.tapl.typing.create_class(cls=Circle, init_args=[s0.Float], methods=[])
""".strip()
    )
...
class Dog:
    ...
    def bark(self):
        ...
        s1.tapl.typing.add_return_type(s1, s1.Str)
        return s1.tapl.typing.get_return_type(s1)
s0.Dog = s0.tapl.typing.create_class(cls=Dog, init_args=[], methods=[('bark', [])])
""".strip()
    )
