...
class SimplestClass:
    pass
s0.SimplestClass = s0.tapl_typing.create_class(cls=SimplestClass, init_args=[], methods=[])

def accept(param):
    s1 = s0.tapl_typing.create_scope(parent__sa=s0, param=param)
    pass
    return s1.tapl_typing.get_return_type(s1)
s0.accept = s0.tapl_typing.create_function([s0.SimplestClass.result__sa], accept(s0.SimplestClass.result__sa))
...
class Circle:
    ...
    def is_bigger_than(self, radius):
        ...
        return s1.tapl_typing.get_return_type(s1)
s0.Circle = s0.tapl_typing.create_class(cls=Circle, init_args=[s0.Float], methods=[('area', []), ('set_radius', [s0.Float]), ('is_bigger_than', [s0.Float])])
