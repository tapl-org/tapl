...
from tapl_lang.lib import codegen, compiler, proxy, scope, terms, untyped_terms
...
def parse_stmt(text: str, *, debug=False) -> list[ast.stmt]:
    ...
    return [s for layer in layers for s in codegen.generate_stmt(layer, syntax.AstSetting(scope_level=0))]
...
def parse_module(text: str) -> list[ast.AST]:
    ...
    return [codegen.generate_ast(layer, syntax.AstSetting(scope_level=0)) for layer in layers]
