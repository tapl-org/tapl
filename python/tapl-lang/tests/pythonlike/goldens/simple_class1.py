...
class SimplestClass:
    pass
s0.SimplestClass = predef.create_scope_proxy(label__tapl='SimplestClass')
s0.SimplestClass_ = predef.create_scope_proxy(label__tapl='SimplestClass_')
s0.SimplestClass.__call__ = predef.FunctionType([s0.SimplestClass], s0.SimplestClass_)

def accept(param):
    s1 = predef.create_scope_proxy(s0, param=param)
    pass
...
class Circle:
    ...
    def area(self):
        s1 = predef.create_scope_proxy(s0, self=self)
...
def print_area(circle):
    s1 = predef.create_scope_proxy(s0, circle=circle)
