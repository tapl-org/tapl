...
def accept(param):
    s1 = s0.tapl_typing.create_scope(parent__tapl=s0, param=param)
    pass
...
class Circle_:
    ...
    def area(self):
        s1 = s0.tapl_typing.create_scope(parent__tapl=s0, self=self)
...
def print_area(circle):
    s1 = s0.tapl_typing.create_scope(parent__tapl=s0, circle=circle)
    s1.tapl_typing.print_log(s1.circle.area())
    return s1.tapl_typing.get_return_type(s1)
