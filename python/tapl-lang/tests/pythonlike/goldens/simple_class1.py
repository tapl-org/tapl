...
class SimplestClass_:
    pass
s0.SimplestClass, s0.SimplestClass_ = s0.api__tapl.create_class(cls=SimplestClass_, init_args=[], methods=[])

def accept(param):
    s1 = s0.api__tapl.create_scope(parent__tapl=s0, param=param)
    pass
    return s1.api__tapl.get_return_type(s1)
s0.accept = s0.api__tapl.create_function([s0.SimplestClass], accept(s0.SimplestClass))
...
class Circle_:
    ...
    def area(self):
        ...
        return s1.api__tapl.get_return_type(s1)
s0.Circle, s0.Circle_ = s0.api__tapl.create_class(cls=Circle_, init_args=[s0.Float], methods=[('area', [])])

def print_area(circle):
    s1 = s0.api__tapl.create_scope(parent__tapl=s0, circle=circle)
    s1.api__tapl.print_log(s1.circle.area())
    return s1.api__tapl.get_return_type(s1)
s0.print_area = s0.api__tapl.create_function([s0.Circle], print_area(s0.Circle))
